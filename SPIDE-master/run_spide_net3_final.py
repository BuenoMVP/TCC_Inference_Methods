#!/usr/bin/env python3
"""
Script para executar o SPIDE com dados do Net3
"""
...
def convert_net3_to_spide_format():
    ...
    # Ler dados de expressão gênica do Net3
    print("Lendo dados de expressão gênica do Net3...")
...
def create_ppi_network_net3():
    """
    Cria uma rede PPI baseada nos genes do Net3
    """
    ...
    # Criar rede PPI mais realista para Net3
    ppi_edges = []
    ...
    # Adicionar conexões baseadas em proximidade (genes próximos se conectam)
    for i in range(len(gene_ids)):
        for j in range(i+1, min(i+5, len(gene_ids))):  # Conecta com até 4 genes próximos
            if np.random.random() < 0.3:  # 30% de chance de conexão
                ppi_edges.append([gene_ids[i], gene_ids[j]])
                ppi_edges.append([gene_ids[j], gene_ids[i]])  # Bidirecional
    
    # Adicionar algumas conexões aleatórias
    np.random.seed(42)  # Para reprodutibilidade
    n_random_edges = min(200, len(gene_ids) * 3)
...
def run_spide_net3():
    ...
    ppi_file = create_ppi_network_net3()
    
    # Parâmetros do SPIDE
    k = 15  # Número de k-vizinhos mais próximos (reduzido para Net3)
    ncore = 2  # Número de cores para processamento paralelo (reduzido)
