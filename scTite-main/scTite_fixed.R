{
  library(data.table)
  library(expm)
  # library(ggm) # Removido - não disponível
  library(lpSolve)
  library(princurve)
  library(mclust)
  library(umap)
...
# Função alternativa para pcor (correlação parcial) sem dependência do pacote ggm
pcor <- function(vars, cov_matrix) {
  # Implementação simples de correlação parcial
  # Para uma implementação mais robusta, seria necessário usar inversão de matriz
  # Esta é uma versão simplificada
  if (length(vars) == 2) {
    # Correlação simples entre duas variáveis
    return(cov_matrix[vars[1], vars[2]] / sqrt(cov_matrix[vars[1], vars[1]] * cov_matrix[vars[2], vars[2]]))
  } else {
    # Para múltiplas variáveis, usar correlação simples como aproximação
    return(cor(cov_matrix[vars[1], ], cov_matrix[vars[2], ]))
  }
}
