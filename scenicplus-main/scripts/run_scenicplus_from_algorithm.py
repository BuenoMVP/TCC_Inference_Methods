#!/usr/bin/env python3
"""
Script para executar análise SCENIC+ a partir da pasta do algoritmo
"""
...
def load_converted_data():
    """
    Carrega os dados convertidos da pasta do algoritmo
    """
    ...
    expression_file = data_dir / "expression_matrix.tsv"
    expression_matrix = pd.read_csv(expression_file, sep='\t', index_col=0)
    
    # Carregar matriz ATAC
    atac_file = data_dir / "atac_matrix.tsv"
    atac_matrix = pd.read_csv(atac_file, sep='\t', index_col=0)
    
    # Carregar metadados
    cell_metadata = pd.read_csv(data_dir / "cell_metadata.tsv", sep='\t', index_col=0)
    gene_metadata = pd.read_csv(data_dir / "gene_metadata.tsv", sep='\t', index_col=0)
    region_metadata = pd.read_csv(data_dir / "region_metadata.tsv", sep='\t', index_col=0)
...
def save_results(tf_to_gene_df, region_to_gene_df, eregulons_df, 
                direct_auc_matrix, extended_auc_matrix):
    """
    Salva os resultados da análise na pasta do algoritmo
    """
    ...
    # Salvar adjacências
    tf_to_gene_df.to_csv(output_dir / "tf_to_gene_adjacencies.tsv", sep='\t', index=False)
    region_to_gene_df.to_csv(output_dir / "region_to_gene_adjacencies.tsv", sep='\t', index=False)
    
    # Salvar eRegulons
    direct_eregulons = eregulons_df[~eregulons_df['is_extended']]
    extended_eregulons = eregulons_df[eregulons_df['is_extended']]
    
    direct_eregulons.to_csv(output_dir / "eRegulon_direct.tsv", sep='\t', index=False)
    extended_eregulons.to_csv(output_dir / "eRegulons_extended.tsv", sep='\t', index=False)
    
    # Salvar scores AUCell
    direct_auc_matrix.to_csv(output_dir / "AUCell_direct_scores.tsv", sep='\t')
    extended_auc_matrix.to_csv(output_dir / "AUCell_extended_scores.tsv", sep='\t')
...
def generate_summary_report(eregulons_df, direct_auc_matrix, extended_auc_matrix):
    """
...
- **Score médio direto**: {direct_auc_matrix.values.mean():.3f if not direct_auc_matrix.empty else 'N/A'}
...
"""
    ...
    # Salvar relatório
    with open(output_dir / "SCENIC_analysis_report.md", 'w') as f:
...
def main():
    ...
    Função principal
    """
    print("🧬 Análise SCENIC+ - Pasta do Algoritmo")
    print("=" * 50)
    
    try:
        # 1. Carregar dados convertidos
        expression_matrix, atac_matrix, cell_metadata, gene_metadata, region_metadata = load_converted_data()
        ...
        # 7. Gerar relatório
        generate_summary_report(eregulons_df, direct_auc_matrix, extended_auc_matrix)
        
        print("\n" + "=" * 50)
        print("✅ Análise SCENIC+ concluída com sucesso!")
        print(f"📁 Resultados salvos em: {project_root}/data/output/")
