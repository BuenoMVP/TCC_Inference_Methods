#!/usr/bin/env python3
"""
Script para demonstrar os resultados da análise SCENIC+
"""
...
def load_results(results_dir):
    """
    Carrega todos os resultados da análise SCENIC+
    """
    print("📊 Carregando resultados da análise SCENIC+...")
    
    # Carregar arquivos de resultados
    tf_to_gene = pd.read_csv(f"{results_dir}/tf_to_gene_adjacencies.tsv", sep='\t')
    region_to_gene = pd.read_csv(f"{results_dir}/region_to_gene_adjacencies.tsv", sep='\t')
    direct_eregulons = pd.read_csv(f"{results_dir}/eRegulon_direct.tsv", sep='\t')
    extended_eregulons = pd.read_csv(f"{results_dir}/eRegulons_extended.tsv", sep='\t')
    
    # Carregar scores AUCell se existirem
    try:
        direct_auc = pd.read_csv(f"{results_dir}/AUCell_direct_scores.tsv", sep='\t', index_col=0)
    except:
        direct_auc = pd.DataFrame()
    
    try:
        extended_auc = pd.read_csv(f"{results_dir}/AUCell_extended_scores.tsv", sep='\t', index_col=0)
    except:
...
def main():
    ...
    Função principal
    """
    print("🧬 Demonstração dos Resultados SCENIC+")
    print("=" * 60)
    
    try:
        # Diretório de resultados relativo
        project_root = Path(__file__).parent.parent
        results_dir = project_root / "data" / "output"
        
        # Carregar resultados
        tf_to_gene, region_to_gene, direct_eregulons, extended_eregulons, direct_auc, extended_auc = load_results(results_dir)
